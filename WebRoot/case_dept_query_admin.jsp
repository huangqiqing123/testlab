...
<script type="text/javascript">
	...
	function update() {
		...
		if (obj != null) {
			...
			document.form2.action = "casedo.do?method=forupdate&path=case_dept_query_admin.jsp";
	...
	function del() {
		...
		if (obj != null) {
			...
			if (confirm('确认删除选中记录？文件删除后将不能恢复！')) {
				document.form2.action = "casedo.do?method=delete&path=case_dept_query_admin.jsp";
				document.form2.submit();
			}
		}
	}
	...
	function query() {
		...
		document.formQuery.action = "casedo.do?method=query&path=case_dept_query_admin.jsp&pageSize="+pageSize;
...
<script language="javascript" src="js/pub.js"></script>
	...
	<tr>
	<td>
		<form name="formQuery" action="casedo.do?method=query&path=case_dept_query_admin.jsp" method="post">
		<fieldset>
			<legend>查询条件</legend>		
<%
 ...
 %>	
			<table align="center" title="双击可清除查询条件！" cellpadding="1" cellspacing="0" border="0">
				...
				<td nowrap="nowrap">
					关键字
					<input type="text" id="title" name="title" value="<%=query_condition.getTitle()%>" size="30" ondblclick="clear_condition(this)" >
				...
				<td nowrap="nowrap">
					上传者
					<input type="text" id="upload_person" name="upload_person" value="<%=query_condition.getUpload_person()%>" size="30" ondblclick="clear_condition(this)" >
				</td>
				</tr>
				<tr>
				<td nowrap="nowrap">
			...
			<legend>
					...
					<tr>
						...
						<th id="title" width="400"   onclick="sort(this)"><a style="CURSOR:hand">文件名称<span id='title_gif'></span></a></th>
						<th id="type" nowrap="nowrap" onclick="sort(this)" ><a style="CURSOR:hand">类型<span id='type_gif'></span></a></th>
						<th id="status" onclick="sort(this)" nowrap="nowrap" ><a style="CURSOR:hand">状态<span id='status_gif'></span></a></th>
						<th id="upload_person" onclick="sort(this)" nowrap="nowrap" width="80"><a style="CURSOR:hand">上传者<span id='upload_person_gif'></span></a></th>
			...
			<form name="public_info" action="casedo.do?method=query&path=case_dept_query_admin.jsp" method="post" >
					<input type="hidden" name="sort" value=<%=query_condition.getSort()%>>
					<input type="hidden" name="sortType" value=<%=query_condition.getSortType()%>>
