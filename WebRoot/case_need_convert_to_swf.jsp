...
function auto() {
	showTips();
	window.location.href="casedo.do?method=autoConvert";
}
//显示提示信息
function showTips(){
...
//刷新
function refresh() {
	window.location.href="case_need_convert_to_swf.jsp";
}
</script>
	</head>
		...
		<input type="button" value="刷新" class="btbox" onclick="refresh()">
			...
			<tr><td ><fieldset><legend>待转换文件列表</legend>	
					...
					*/
					...
					List<Case> list = caseDao.noPageQueryByStauts("4");	
				...
				%>
					...
					<td align="center" >
						...
						<u style="CURSOR:hand" onclick="window.location.href='casedo.do?method=download&pk=<%=view.getPk() %>'">
							<%=view.getBlob_name()%>
						</u>
						</span>
						</td>
						...
						<td>
							<input type="button" value="转换" class="btbox" onclick="showTips();window.location.href='casedo.do?method=convertToSWFwinthSingle&suffix=<%=suffix%>&pk=<%=view.getPk() %>'">
						</td>
					</tr>
					<%} %>
				</table>
