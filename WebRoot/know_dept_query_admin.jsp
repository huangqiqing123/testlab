...
<script type="text/javascript">
	...
	function update() {
		...
		if (obj != null) {
			...
			document.form2.action = "knowdo.do?method=forupdate&path=know_dept_query_admin.jsp";
	...
	function del() {
		...
		if (obj != null) {
			...
			if (confirm('确认删除选中记录？文件删除后将不能恢复！')) {
				document.form2.action = "knowdo.do?method=delete&path=know_dept_query_admin.jsp";
				document.form2.submit();
			}
		}
	}
	...
	function query() {
		...
		document.formQuery.action = "knowdo.do?method=query&path=know_dept_query_admin.jsp&pageSize=" + pageSize;
...
<script language="javascript" src="js/suggest.js"></script>
	...
	<tr>
	<td>
		<form name="formQuery" action = "knowdo.do?method=query&path=know_dept_query_admin.jsp" method="post">
		<fieldset>
			<legend>查询条件</legend>		
<%
 ...
 %>		
				<table align="center" title="双击可清除查询条件！" cellpadding="1" cellspacing="0" border="0">
					...
					<td nowrap="nowrap">
					关键字
					<input type="text"  id="title" name="title" value="<%=query_condition.getTitle()%>" size="35" ondblclick="clear_condition(this)" onkeyup="sendRequest('know_dept_query_admin.jsp')" onclick="hiddenSuggest()">
			...
			<form name="public_info" action="knowdo.do?method=query&path=know_dept_query_admin.jsp" method="post" >
					<input type="hidden" name="sort" value=<%=query_condition.getSort()%>>
					<input type="hidden" name="sortType" value=<%=query_condition.getSortType()%>>
