...
<script type="text/javascript">
	...
	function query() {
		...
		document.formQuery.action = "knowdo.do?method=query&path=know_dept_query_user.jsp&pageSize="+pageSize;
...
<script language="javascript" src="js/suggest.js"></script>
	...
	<tr>
	<td>
		<form name="formQuery" action="knowdo.do?method=query&path=know_dept_query_user.jsp" method="post">
		<fieldset>
			<legend>查询条件</legend>		
<%
 ...
 %>	
				...
				<table align="center"  cellpadding="1" cellspacing="0" border="0">
					...
					<td>
					<input type="text" id="title" name="title" value="<%=query_condition.getTitle()%>" size="55" ondblclick="clear_condition(this)" onkeyup="sendRequest('know_dept_query_user.jsp')" onclick="hiddenSuggest()" >
					</td>
					<td>
					...
					<!-- 下面的iframe是用来解决IE6下，div层在select下拉框层的下方，导致展现列表不能正常展现问题的。 -->
			...
			<form name="public_info" action="knowdo.do?method=query&path=know_dept_query_user.jsp" method="post" >
					<input type="hidden" name="sort" value=<%=query_condition.getSort()%>>
					<input type="hidden" name="sortType" value=<%=query_condition.getSortType()%>>
