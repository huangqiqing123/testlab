...
<script language="javascript">
	...
	function check_status() {
		if (addForm.status.value == "") {
			$("status_check_result").innerHTML = "<font color='red'>提示：请选择文件状态！</font>";
			return false;
		} else {
			$("status_check_result").innerHTML = "<font color='green'>OK</font>";
			return true;
		}
	}
	//执行保存
	function save() {
		if (check_type()&&check_status()&&check_blob()&&check_title()&&checkmemolength('memo')) {
			document.addForm.action = "knowdo.do?method=update&action=save&path=know_update_admin.jsp";
...
<table align="center" border="1" width="100%" cellpadding="1" cellspacing="0">
	<tr>
		...
		<td align="left">
		<select name="status" size="1" onblur="check_status()"	onchange="check_status()">
			<option value="" />---请选择--- 
			<%
				for (Map.Entry<String, String> entry:know_status.entrySet()) {
			%>
			<option value="<%=entry.getKey()%>" <%
			if(entry.getKey().equals(view.getStatus())){
				out.print("selected=\"selected\"");
			}
			%>/><%=entry.getValue()%> 
		<%}%>
		...
		<font color="red">*</font> 
		<div id="status_check_result"></div></td>
	</tr>
	<tr>
