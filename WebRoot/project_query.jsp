...
<script type="text/javascript">
	...
	function del() {
		...
		if (obj != null) {
			...
			if (confirm('确认删除选中记录？')) {
				document.form2.action = "projectdo.do?method=delete&isLabProject=0";
				document.form2.submit();
			}
		}
	}
...
<table width="100%" height="100%" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td>
		<h2 align="center">非实验室项目信息管理</h2>
		</td>
	</tr>
	<tr>
		<td align="right"><input type="button" class="btbox" value="增加"
			onClick="window.location.href='project_add.jsp?path=project_query.jsp'"
			<%
			if (!Tool.isDocmentAdmin(request)&&!Tool.isSuperadmin(request)&&!Tool.isFunctionManager(request)) {
	...
	<tr>
		...
		<%
			...
			String testmanager = query_condition.getTestmanager();
		%>
		<table align="center" cellpadding="1" cellspacing="0" border="0">
			<tr>
				...
				<td>测试经理</td>
				<td><input type="text" name="testmanager" size="15" maxlength="50"	value="<%=testmanager == null ? "" : testmanager%>"  ondblclick="clear_condition(this)"></td>
				
				<td colspan="3">&nbsp;
				<input type="hidden" name="sort"  value="<%=sort == null ? "code" : sort%>">
				<input type="hidden" name="sortType"  value="<%=sortType == null ? "ASC" : sortType%>">
				<input type="hidden" name="isLabProject"  value="0"><!--0表示非实验室项目  -->
				<input type="hidden" name="path" value="project_query.jsp">
				<input type="hidden" name="code" size="20" maxlength="32"	value="<%=code == null ? "" : code%>"  ondblclick="clear_condition(this)">
				</td>
			</tr>
			<tr>
			<td>项目类别</td>
 ...
 %>
	...
	<tr>
		...
		<table border="1" cellpadding="1" cellspacing="0" align="center" width="100%" >
			...
			<tr>
				...
				<th  nowrap="nowrap" onclick="sort(this)" id="project_type"><a style="CURSOR:hand">项目类别<span id='project_type_gif'></span></a></th>
				<th nowrap="nowrap" onclick="sort(this)" id="project_customer"><a style="CURSOR:hand">客户名称<span id='project_customer_gif'></span></a></th>
				<th nowrap="nowrap" onclick="sort(this)" id="testmanager"><a style="CURSOR:hand">测试经理<span id='testmanager_gif'></span></a></th>
			...
			<tr id="line<%=i %>" >
				...
				<td  id="<%=i%>" onclick="clickLine(this)"><a href="projectdo.do?method=detail&pk=<%=project.getPk()%>"><%=project.getName()%></a></td>
				<td nowrap="nowrap" id="<%=i%>" onclick="clickLine(this)"><%=project_types.get(project.getProject_type())%></td>
				<td nowrap="nowrap" id="<%=i%>" onclick="clickLine(this)"><%=project_customers.get(project.getProject_customer())%></td>
				<td nowrap="nowrap" id="<%=i%>" onclick="clickLine(this)"><%=project.getTestmanager() %></td>
	...
	<tr>
		...
		<input type="hidden" name="isLabProject" value="0">
		<input type="hidden" name="path" value="project_query.jsp">
		...
		<input type="hidden" name="testmanager" value=<%=testmanager%>> 
